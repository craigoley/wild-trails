...
describe('Journal panel — grouped by biome', () => {
  it('renders all 5 biome headers, inside the scroll body, with the overall total kept', () => {
    ...
    expect(document.querySelectorAll('.journal-biome')).toHaveLength(16); // all 16 biomes (+ §savanna African Savanna) (+ §desert Sonoran Desert) shown (+ §migration Estuary) (+ §hedgerow Hedgerow + Hazel Copse) (+ Alpine Summit, §4.2)
    ...
    expect(document.querySelector('.journal-title')!.textContent).toContain('0 of 76');
  });
