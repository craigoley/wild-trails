...
describe('Journal panel — grouped by biome', () => {
  it('renders all 5 biome headers, inside the scroll body, with the overall total kept', () => {
    ...
    expect(document.querySelectorAll('.journal-biome')).toHaveLength(9); // all 9 biomes shown (+ Pine Forest, §4.2)
    ...
    expect(document.querySelector('.journal-title')!.textContent).toContain('0 of 39');
  });
