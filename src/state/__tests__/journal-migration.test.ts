 ...
 */
const FULL_BAIT = { seeds: BAIT.startingCount, greens: BAIT.startingCount, insects: BAIT.startingCount, fish: 0, shellfish: 0, meat: 0 }; // fish + shellfish are research-gated -> start at 0
...
describe('Journal — v6 -> v7 (the new hop: add empty research, keep ALL v6 data)', () => {
  it('preserves species/nets/credits/won and adds research at its empty default', () => {
    ...
    expect(out.bait).toEqual({ seeds: 2, greens: 5, insects: 9, fish: 0, shellfish: 0, meat: 0 }); // all v3-v6 data kept; fish + shellfish default 0
...
describe('Journal — a current v7 store round-trips exactly', () => {
  it('keeps credits + won + nets + research through a round-trip', () => {
    const v7Store = {
      ...
      rankPoints: 30,
      unlockedBiomes: ['woodland'],
      bait: { seeds: 2, greens: 5, insects: 9, fish: 0, shellfish: 0, meat: 0 },
      won: true,
      credits: 42,
      ownedTools: ['net'],
      activeTool: 'net',
